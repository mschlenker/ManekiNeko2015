...
void loop() {
  ...
  if (val > 0) {
    ...
    if (cyclesLeft > 0) {
      cyclesLeft -= 1;
      delay( ( MINPAUSE + (uptime % ( MAXPAUSE - MINPAUSE ))) * 10 + 3 );
      analogWrite(MOTPIN, MOTOR_DUTY_CYCLE);
    } else {
      // we have just reached the last cycle, switch off everything
      stopPlayer(); 
      lightOff(); 
      setPwmFrequency(MOTPIN, 64);
      delay(INTERPAUSE);
      duration = 1000000;
      while (duration > ULTRATIME) {
